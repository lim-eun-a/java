...
public class Human
{
	// 멤버변수: 객체의 속성을 표현
	String name;
	// 나이의 경우 연산이 필요하지 않다면 String으로 설정해도 무방하다.
	int age;
	...
	void walking() {
		...
		energy--;
		/*
		 만약 에너지를 차감한 결과가 음수라면 해당 시나리오의 조건에 만족하지 않으므로
		 0으로 고정하는 코드가 필요
		 */
		if(energy<0) {
			// 차람 후 만약 음수가 되면 0으로 고정
			energy = 0;
	...
	void eating() {
		...
		if(energy>10) {
			...
			System.out.println("[thinking]에너지가 2 증가하였습니다.");
		}		
	}
