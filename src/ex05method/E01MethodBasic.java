...
/*
메서드(함수)의 규칙
- Java에서 main메서드는 무조건 public static void로 선언하기로 약속되어 있다.
- static으로 선언된 메서드는 static으로 선언된 메서드만 호출할 수 있다.
- 메서드 호출 후 반환값은 호출한 위치로 반환된다. 이때 메서드는 메모리에서 소멸된다.
- 반환값이 없는 void형 메서드는 실행이 완료되면 호출한 위치로 실행의 흐름이 돌아온다.
 */

public class E01MethodBasic
{
	/*
	 메서드1: 반환값0, 매개변수0 형태로 정의
	 	=> 매개변수 2개의 정수를 전달받아 합의 결과를 반환한다. 
	 		반환값은 호출한 지점으로 반환한다.
	 */
	...
	/*
	 메소드2: 반환값X, 매개변수X 형태로 정의
	 	=> 전달되는 값 없이 이름을 입력받은 후 namePrint()메서드를 호출한다.
	 	   이와값이 반환값이 없는 경우에는 반드시 void를 명시해야 한다.
	 */
	...
	public static void main(String[] args)
	{
		...
		menuPrint();
		//main()메서드의 끝을 만나면 프로그램 자체가 종료된다.
	}
