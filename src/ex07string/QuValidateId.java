...
public class QuValidateId
{
	...
	static boolean idValidate(String inputId) 
	{
		...
		if(inputId.length()>=8 && inputId.length()<=12) 
		{
			...
			for(int i=0; i<inputId.length(); i++) 
			{
				...
				// 아이디가 영문 또는 숫자가 아닌 경우 false로 변경해준다.
				if(!((idChr>='0' && idChr<='9') || (idChr>='a' && idChr<='z') 
						|| (idChr>='A' && idChr<='Z')))
				{ 
					System.out.println(idChr + " ==> 허용X.");
					idOk = false;
					// 문자열에서 허용되지 않는 문자를 찾았다면 
					// 뒷부분은 검사할 필요가 없으므로 즉시 반복문(for)을 탈출.
					break;
				}
			}
		} else 
		{
			// 아이디 길이가 잘못되었다면 false로 변경한다.
			idOk = false;
		}
		return idOk;
	}
	...
	public static void main(String[] args)
	{
		...
		if(idOk==true) {
			...
			System.out.println("사용할 수 없습니다. 다시 입력해 주세요");
		}
	}
