...
public class QuLottoArray
{
	...
	public static void main(String[] args)
	{
		...
		int[] arr = new int[6];
		// 난수 생성 후 arr 배열에 저장하기 위한 메소드 호출
		randomCreate(arr);
		
		// 2) 생성된 난수를 통해 초기화 된 배열을 출력한다.(정렬 전 출력)
		showArray(arr, "난수 생성 직후 로또번호 출력");
	...
	public static void bubbleSort(int[] arrParam, int ordSel)
	{
		...
		/*
		 	크기가 6인 배열이므로 스캔은 5번만 반복하면 된다.
		 	즉, i의 증가치는 0~5까지 변하게 된다.
		 */
		for (int i = 0; i < arrParam.length-1; i++)
		{
			/*
			 	실제 요소값에 대한 비교를 진행한다.
			 	버블정렬의 목적은 오름차순일 때 큰 숫자를 제일 뒤로 보내주는 것이다.
			 	즉, 요소1과 요소2를 비교하여 요소1이 크다면 서로 자리를 바꿔서
			 	큰 숫자를 뒤로 보내준다.
			 */
			for (int j = 0; j < (arrParam.length-1)-i; j++)
			{
				/*
				 	j가 0일 때: arrParam[0] > arrParam[1] 비교
				 	j가 1일 때: arrParam[1] > arrParam[2] 비교
				 	.....
				 */
				if(ordSel==1)	// 오름차순을 선택했을 때
				{
					if(arrParam[j] > arrParam[j+1])
					{
						/*
						 	앞에 요소가 크다고 판단이 되면 서로 swap하여 
						 	자리를 바꿔서 큰 숫자를 뒤로 보내준다.
						 */
						temp = arrParam[j];
						arrParam[j] = arrParam[j+1];
						arrParam[j+1] = temp;
						
						// 값을 변경이 있을 때마다 배열 전체 출력하기
//						showArray(arrParam,"SWAP중(오름차순 정렬 중): " 
//											+ (swapCount++) + "회 교환됨");
					}
				} else if(ordSel==2)	// 내림차순을 선택했을 때
				{
					if(arrParam[j] < arrParam[j+1])
					{
						/*
						 	앞에 요소가 작다고 판단이 되면 서로 swap하여 
						 	자리를 바꿔서 작은 숫자를 뒤로 보내준다.
						 */
						temp = arrParam[j];
						arrParam[j] = arrParam[j+1];
						arrParam[j+1] = temp;
						...
						// 값을 변경이 있을 때마다 배열 전체 출력하기
//						showArray(arrParam,"SWAP중(내림차순 정렬 중): " 
//											+ (swapCount++) + "회 교환됨");
					}
				}
			}
	...
	public static void randomCreate(int[] arrParam)
	{
		...
		while(true) {
			...
			for(int i=0; i<arrParam.length; i++) {
				arrParam[i] = rnd.nextInt(45)+1; // 1~45까지의 정수가 생성
			...
			boolean rndFlag = false; 
				/*
					중복 체크를 위한 변수(false)라면 중복된 난수가 없는 경우이고,
					중복된 난수가 발견된다면 true로 값을 변경할 것임
				*/
			for (int i = 0; i < arrParam.length; i++)
			{
				...
				{
					/*
					 	비교의 조건은 인덱스 i와 인덱스 j가 서로 다를 때이다.
